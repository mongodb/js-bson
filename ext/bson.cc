...
uint32_t BSON::calculate_object_size(BSON *bson, Handle<Value> value, bool serializeFunctions) {
  ...
  if(value->IsString()) {
    ...
    if(serializeFunctions) {
     ...
     object_size += buffer->length() + 4 + 1;
    #else
     object_size += Buffer::Length(value->ToObject()) + 4 + 1;
    #endif
...
uint32_t BSON::serialize(BSON *bson, char *serialized_object, uint32_t index, Handle<Value> name, Handle<Value> value, bool check_key, bool serializeFunctions) {
  ...
  if(value->IsString()) {
    ...
    *(serialized_object + index) = '\0';    
    ...
    #else
     data = Buffer::Data(value->ToObject());
     length = Buffer::Length(value->ToObject());
    #endif
    
    // Save the string at the offset provided
    *(serialized_object + index) = BSON_DATA_BINARY;
    // Write the size of the buffer out
    BSON::write_int32((serialized_object + index), length);
    // Adjust index
    index = index + 4;
    // Write subtype
    *(serialized_object + index)  = (char)0x00;
    // Adjust index
    index = index + 1;
    // Write binary content
    memcpy((serialized_object + index), data, length);
