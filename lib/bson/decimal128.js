...
const BufferConstructor = typeof Buffer !== 'undefined' ? Buffer : Uint8Array;
...
Decimal128.fromString = function(string) {
  ...
  if (!isDigit(string[index]) && string[index] !== '.') {
    if (string[index] === 'i' || string[index] === 'I') {
      return new Decimal128(
        new BufferConstructor(isNegative ? INF_NEGATIVE_BUFFER : INF_POSITIVE_BUFFER)
      );
    } else if (string[index] === 'N') {
      return new Decimal128(new BufferConstructor(NAN_BUFFER));
    }
  }
  ...
  if (string[index] === 'e' || string[index] === 'E') {
    ...
    // No digits read
    if (!match || !match[2]) return new Decimal128(new BufferConstructor(NAN_BUFFER));
  ...
  // Return not a number
  if (string[index]) return new Decimal128(new BufferConstructor(NAN_BUFFER));
  ...
  var buffer = new BufferConstructor(16);
  index = 0;
