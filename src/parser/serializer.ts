...
function serializeString(buffer: Uint8Array, key: string, value: string, index: number) {
  ...
  const numberOfWrittenBytes = setUtf8String(buffer, key, index);
  // Encode the name
  index = index + numberOfWrittenBytes + 1;
  buffer[index - 1] = 0;
  // Write the string
  const size = setUtf8String(buffer, value, index + 4);
...
function serializeRegExp(buffer: Uint8Array, key: string, value: RegExp, index: number) {
  ...
  // Adjust the index
  index = index + setUtf8String(buffer, value.source, index);
  // Write zero
  buffer[index++] = 0x00;
...
function serializeBSONRegExp(buffer: Uint8Array, key: string, value: BSONRegExp, index: number) {
  ...
  // Adjust the index
  index = index + setUtf8String(buffer, value.pattern, index);
  // Write zero
  buffer[index++] = 0x00;
  // Write the options
  const sortedOptions = value.options.split('').sort().join('');
  index = index + setUtf8String(buffer, sortedOptions, index);
  // Add ending zero
  buffer[index++] = 0x00;
  return index;
}
...
function serializeObjectId(buffer: Uint8Array, key: string, value: ObjectId, index: number) {
  ...
  if (typeof value.id === 'string') {
    // TODO we should never enter this block anymore....
    const bytes = ByteUtils.fromISO88591(value.id);
...
function serializeFunction(
  ...
  // Write the string
  const size = setUtf8String(buffer, functionString, index + 4) + 1;
  // Write the size of the string to buffer
  buffer[index] = size & 0xff;
...
function serializeCode(
  ...
  if (value.scope && typeof value.scope === 'object') {
    ...
    const codeSize = setUtf8String(buffer, functionString, index + 4) + 1;
    // Write the size of the string to buffer
...
function serializeSymbol(buffer: Uint8Array, key: string, value: BSONSymbol, index: number) {
  ...
  buffer[index++] = 0;
  // Write the string
  const size = setUtf8String(buffer, value.value, index + 4) + 1;
  // Write the size of the string to buffer
  buffer[index] = size & 0xff;
  buffer[index + 1] = (size >> 8) & 0xff;
  buffer[index + 2] = (size >> 16) & 0xff;
