...
var BSON = require('../..'),
  ...
  scenariosCorrupt = require('./compliance/corrupt'),
  scenariosValid = require('./compliance/valid');
...
describe('BSON Compliance', function() {
  ...
  it('Pass all corrupt BSON scenarios ./compliance/corrupt.json', function(done) {
    ...
    for (var i = 0; i < scenariosCorrupt.documents.length; i++) {
      var doc = scenariosCorrupt.documents[i];
  ...
  it('Pass all valid BSON serialization scenarios ./compliance/valid.json', function(done) {
    ...
    scenariosValid.documents.forEach(function(doc) {
      if (doc.skip) return;
      // Create a buffer containing the payload
